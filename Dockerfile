...
RUN sudo git clone https://github.com/sn1k/Vinos.git
...
RUN ls
RUN cd Vinos/ && ls -l
RUN cd Vinos/ && cat requirements.txt
RUN cd Vinos/ && sudo pip install -r requirements.txt


#Migraciones
RUN cd Vinos && python manage.py syncdb --noinput
